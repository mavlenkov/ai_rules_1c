# Отчёт: находки при тестировании на проекте TestBaseExt

**Дата:** 2026-03-02
**Тестовый проект:** TestBaseExt (расширение конфигурации ЛИС 1С)
**Стенд:** dbsrvhost:1541\TestBase
...
/opt/1cv8/x86_64/8.3.27.1859/1cv8 DESIGNER \
  /IBConnectionString 'Srvr="dbsrvhost:1541";Ref="TestBase";' \
  /NАдминистратор /DisableStartupDialogs /DisableStartupMessages \
  /LoadConfigFromFiles . -Extension TestBase /Out /tmp/1c/ext_load.log

# Обновление структуры БД расширения
/opt/1cv8/x86_64/8.3.27.1859/1cv8 DESIGNER \
  /IBConnectionString 'Srvr="dbsrvhost:1541";Ref="TestBase";' \
  /NАдминистратор /DisableStartupDialogs /DisableStartupMessages \
  /UpdateDBCfg -Extension TestBase /Out /tmp/1c/ext_updatedb.log
```
