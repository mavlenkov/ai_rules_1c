...
Bundles are shipped for `cursor`, `claude-code`, `codex`, `opencode`, `kilocode`. The `kimi` adapter currently has no OpenSpec command bundle because Kimi project slash commands are plugin/skill-based rather than a plain commands folder; Kimi users continue to read `openspec/specs/` and `openspec/changes/` directly and invoke OpenSpec workflows through the installed OpenSpec skills. The `other` adapter (universal fallback) has no OpenSpec bundle either. Skipped bundles are recorded in `.ai-rules.json` under `integrations.openspec.bundleSkipped` for transparency.
