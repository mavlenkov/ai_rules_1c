...
3. **Bash channel (fork-only, Linux fallback).** This fork (`mavlenkov/ai_rules_1c`) ships `scripts/install.sh` — a minimal bash+python implementation of the same protocol for Linux/macOS environments where `pwsh` is unavailable. Supports the `cursor`, `claude-code`, `opencode`, `codex`, and `kimi` tools; adds `--host` to substitute the real MCP server host into `localhost` URLs from `content/mcp-servers.json`. Kilo Code and the other newer adapters are intentionally not covered — use `install.ps1` for those.
