...
3. **Bash channel (fork-only, Linux fallback).** This fork (`mavlenkov/ai_rules_1c`) ships `scripts/install.sh` — a bash+python implementation of the same placement protocol for Linux/macOS environments where `pwsh` is unavailable. It supports eight tools: `cursor`, `claude-code`, `opencode`, `codex`, `kimi`, `kilocode`, `qwen`, and `koda`; adds `--host` to override the managed MCP server host (`MCP_HOST` from the target project's `.dev.env`, then `localhost`) and `--publish-url` for `1c-data-mcp`. It does not create `.dev.env` automatically. Use `install.ps1` for `command-code`, `cline`, `pi`, `other`, and the full lifecycle command set.
...
`1c-rules` is a **project-scoped** toolkit. Every supported tool (Cursor, Claude Code, Codex, OpenCode, Kilo Code, Kimi Code CLI, Qwen Code, Command Code, Cline, Pi, Koda CLI, `other`) reads its always-on context from the project root, and supported on-demand artifacts land under project-local tool directories (`.cursor/`, `.claude/`, `.kilo/`, `.codex/`, `.opencode/`, `.kimi-code/`, `.qwen/`, `.commandcode/`, `.cline/`, `.pi/`, `.koda/`, `.kodacli/`, `.ai-agent/`). Installing into a tool's **global CLI configuration directory** (`~/.config/kilo/`, `~/.codex/`, `~/.claude/`, `~/.opencode/`, `~/.kimi-code/`, `~/.qwen/`, `~/.commandcode/`, `~/.cline/`, `~/.pi/`, `~/.kodacli/`, `%APPDATA%\…\<tool>\`, etc.) is **not supported and is forbidden**: such directories are owned by the CLI itself, the adapter targets (`.kilo/commands/`, `.kilo/agents/`, …) do not match what the CLI looks up globally, `AGENTS.md` path rewriting yields broken links, and `.dev.env` / OpenSpec / `.ai-rules.json` have no project to bind to. A "global" install is always wrong, even when the user has not opened a project — there is nothing meaningful for the rules to attach to.

Before doing **any** filesystem operation, resolve the project root:

1. Use the working directory the agent was launched in if it is plausibly a project root (contains `Configuration.xml`, `ConfigurationExtension.xml`, a `src/` / `cf/` source dump, a `.cursor/` / `.claude/` / `.kilo/` / `.codex/` / `.opencode/` / `.qwen/` / `.commandcode/` / `.cline/` / `.pi/` / `.koda/` / `.kodacli/` / `.ai-agent/` directory, a `.git/`, or an existing `AGENTS.md` / `KODA.md` / `.ai-rules.json`).
2. If the working directory **is** a tool CLI config directory (path matches `~/.config/<tool>`, `~/.codex`, `~/.claude`, `~/.opencode`, `~/.qwen`, `~/.commandcode`, `~/.cline`, `~/.pi`, `~/.kodacli`, `%APPDATA%\…\<tool>`, or contains the CLI's own marker files such as `kilo.jsonc`, `config.toml` next to `prompts/`, `package.json` with `@kilocode/plugin` / `@openai/codex` / etc., or `bun.lock` / `package-lock.json` shipped by the CLI installer) — **STOP**. Do not install there.
...
- **Source** — local clone of `1c-rules` if the user pointed at one; otherwise `https://github.com/mavlenkov/ai_rules_1c` (default).
...
1. **Resolve the source.** If only a URL was given, clone it locally (`git clone https://github.com/mavlenkov/ai_rules_1c.git <cache-dir>/1c-rules`) or reuse an existing clone.
...
3. **Bulk-copy directories.** For each active tool, copy whole directories from `content/` to the adapter's target paths in one shell call each. Do **not** open file bodies during the copy:
   ...
   - `content/skills/` → `<skills.copyTo dir>/` (mode `verbatim` — copy **every** skill folder as-is, no transformation). Copy the whole `content/skills/` directory; do **not** cherry-pick a subset. All skills are required, including the non-1C-domain ones (`caveman`, `prompt-enhancer`, `handoff`, `mermaid-diagrams`, `md-to-docx`, `img-grid-analysis`) — `AGENTS.md` references them and silently skipping any of them leaves a degraded ruleset. Use a single directory copy, not per-skill judgement calls.
...
4. **Apply frontmatter operations only where needed.** For sections that have `frontmatter.keep` / `drop` / `rename` / `addIf` / `toolsToPermission`:
   ...
   - **Agents: resolve `modelTier` first (per target client, fork).** Source agent files declare an abstract `modelTier` (`coding` | `analysis` | `light`) instead of a concrete model name. Before applying the adapter ops, replace it with `modelHint: <concrete model>` resolved from the project `.dev.env` **for the client currently being placed**, by cascade: per-client override `SUBAGENT_MODEL_<TIER>__<TOOL>` → bare tier `SUBAGENT_MODEL_<TIER>` → empty. The `<TOOL>` suffix is the adapter id upper-cased with `-`→`_` (`claude-code → CLAUDE_CODE`, `opencode → OPENCODE`, `cursor → CURSOR`, `kilocode → KILOCODE`). Rationale: model names are client-specific — Claude Code / Cursor take short aliases (`sonnet`), OpenCode / Kilo Code need `provider/model` (`zai-coding-plan/glm-5.2`). Legacy 2-tier `.dev.env` files with no `SUBAGENT_MODEL_ANALYSIS` key fall back to `SUBAGENT_MODEL_CODING` for the `analysis` tier.
     - **Format check for `provider/model` clients.** For `opencode` and `kilocode`, if the resolved name contains no `/`, do **not** emit a model field — print a WARNING suggesting `SUBAGENT_MODEL_<TIER>__<TOOL>` instead. A bare alias would silently fall back to the client default and look broken; skipping it is safer. For `claude-code` / `cursor` no `/` check applies.
     - When the resolved value is empty (no override, no bare tier, or `.dev.env` does not exist yet), simply **remove** `modelTier` and emit no model field — the AI client then uses its default model (all parameters are DEFAULTED; never block or re-ask because of them). On first init the bare-tier values may be asked once as part of the `.dev.env` bootstrap prompts — a benchmark-based profile picker offering `Balanced` / `Economy` / `Quality` (see *`.dev.env` bootstrap* below); per-client suffixes are advanced and not prompted. After the resolution, the adapter ops apply as written (`keep`/`rename` reference `modelHint`).
...
- **Inventing Koda executable subagent files or MCP commands.** Koda has no `.koda/agents/*.json`, `/agent`, or `InvokeAgent` contract; `.koda/agents/*.md` are reference prompts only. Use `.koda/skills/`, a root `KODA.md` stub, and `koda mcp add ... --transport http --scope project` commands.
...
iex (irm https://raw.githubusercontent.com/mavlenkov/ai_rules_1c/main/install.ps1)
iex "$(irm https://raw.githubusercontent.com/mavlenkov/ai_rules_1c/main/install.ps1) init"
```
...
git clone https://github.com/mavlenkov/ai_rules_1c.git $tmp
...
Bundles are shipped for `cursor`, `claude-code`, `codex`, `opencode`, `kilocode`, and `kimi`. Codex and Kimi bundles contain Skills only; Kimi's official `.kimi/skills/` output is placed under the adapter's canonical `.kimi-code/skills/`. The adapters `qwen`, `command-code`, `cline`, `pi`, and `other` currently have no OpenSpec bundle. Skipped bundles are recorded in `.ai-rules.json` under `integrations.openspec.bundleSkipped` for transparency.
