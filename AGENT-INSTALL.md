...
4. **Apply frontmatter operations only where needed.** For sections that have `frontmatter.keep` / `drop` / `rename` / `addIf` / `toolsToPermission`:
   ...
   - **Agents: resolve `modelTier` first (per target client).** Source agent files declare an abstract `modelTier` (`reasoning` | `coding` | `light`) instead of a concrete model name. Before applying the adapter ops, replace it with `modelHint: <concrete model>` resolved from the project `.dev.env` **for the client currently being placed**, by cascade: per-client override `SUBAGENT_MODEL_<TIER>__<TOOL>` → bare tier `SUBAGENT_MODEL_<TIER>` → empty. The `<TOOL>` suffix is the adapter id upper-cased with `-`→`_` (`claude-code → CLAUDE_CODE`, `opencode → OPENCODE`, `cursor → CURSOR`, `kilocode → KILOCODE`). Rationale: model names are client-specific — Claude Code / Cursor take short aliases (`sonnet`), OpenCode / Kilo Code need `provider/model` (`zai-coding-plan/glm-5.2`).
