...
**Проблема:** `.cursor/mcp.json`, `.mcp.json`, `opencode.json` закоммичены с локальным хостом `mcp-host`. При ручном копировании пользователь получает чужую инфраструктуру.
