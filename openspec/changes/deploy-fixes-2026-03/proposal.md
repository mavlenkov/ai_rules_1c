...
При тестировании развёртывания на реальном проекте (TestBaseExt, серверная ИБ dbsrvhost:1541\TestBase) обнаружены критические проблемы: Claude Code не подключает MCP-серверы без `"type":"http"`, формат строки подключения `/S` с пробелом приводит к молчаливой ошибке, а команды deploy не поддерживают загрузку расширений конфигурации (`-Extension`). Подробности: `REPORT-2026-03-02-findings.md`.
