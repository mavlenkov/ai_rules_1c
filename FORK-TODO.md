...
**Приватность публичной копии.** Названия клиентских проектов, внутренние
хосты MCP-сервера и СУБД и реальные имена баз обезличены перезаписью истории
2026-08-26 (бэкап: `~/Проекты/ai_rules_1c-pre-rewrite.bundle`). В новых
коммитах такие идентификаторы НЕ называть даже «для примера» — только
...
(`1c-standards`, роутеры вместо тел — `6f5a738`), полный цикл `/initproject` /
`/restore-testbase` / `/build-release` + EDT-ветка и ветка хранилища (`e5037f3`),
канал поддержки `/support` (+ раздел `.dev.env` с `SUPPORT_*`), дефолт
`syntaxcheck_file` и режим полного индекса у чекера (`8690f80`), правки
MCP-доков под актуальные контракты (`fec68d2`, `84fd32e`), `installtools`
после update. Конфликтов 14 файлов / 24 хунка; политика прежняя: форк-структура сохранена, улучшения
апстрима влиты.

- **Help-MCP-роутеры: ПРИНЯТЫ ПОЛНОСТЬЮ (реверс решения, принятого утром
  этого мержа).** Утром три конфликтных правила (`locks-and-transactions`,
  `platform-solutions`, `systematic-debugging`) оставили с полными телами;
  локальный прогон `validate-rules.ps1` показал нарушение инварианта
  «одно правило — одно тело»: тела всех 14 стандартов лежат в
  `content/standards/` (пин апстрима `410951e`), 11 правил уже были роутерами
  через авто-мерж. Решение: три правила переведены на upstream-роутеры,
  модель апстрима принята целиком. Обоснование: коллекция `1c-standards`
  входит в дистрибутив Help MCP и **уже развёрнута во всех 9 проектах**
  (tool `standards`) — самодостаточность inline-тел больше ничего не покупает.
  ⚠ Цена реверса: форк-фикс `НачалоТранзакции → НачатьТранзакцию`
  (`6f2c35d`) ушёл из текстов правил вместе с телами; корпус это чинить не
  обязан — сделать маленький PR в upstream comol с этим фиксом.
  ⚠ CI: в `.github/workflows/validate-rules.yml` добавлен guard
  `if: github.repository == 'comol/ai_rules_1c'` — на форке workflow не
  запускается (письма о прогонах на каждый пуш); валидатор гонять локально:
  `pwsh tools/validate-rules.ps1`.
- **`.dev.env.example`: нумерация разделов.** У апстрима «Раздел 5» — канал
  поддержки; у форка Раздел 5 = Linux+конвертер (ссылки из команд, AGENTS.md).
  Канал поддержки вставлен как **Раздел 6** с пометкой о верхstream-нумерации;
  ссылок на номер секции в апстримских правилах/командах нет (проверено).
  Заодно закрыт латентный пробел: блок EDT-комментариев обещал ключ
  `V8_EDT_VERSION`, но строки `EDT_VERSION=` в файле не было — дописана.
- **Команды деплоя:** в форк-версии влиты full-snapshot-режим и гейты
  апстрима: `EXTENSION_NAMES` / `EXTENSIONS_PATH` + `argumentHint "[all]"`,
  EDT gate, Repository gate (`REPOSITORY_PATH`), Step 3c → секция
  «Applicability check before the DB update» перед retry-loop в
  `deploy-and-test` (нумерация адаптирована под форк-структуру), success-phrase
  классификация в retry-loop, `/test-fix-loop` в финальном отчёте.
  Объединённая таблица ключей в `loadfrom1cbase` / `update1cbase` /
  `deploy-and-test` содержит оба набора строк (конвертер + новые).
- **MCP-команды:** Platform note (Linux) и Release channel секции сосуществуют
  в `installmcp` / `updatemcp` (обе аддитивны); `/checkmcp` авто-мерж принял
  ссылки на канон release-channel.
- **`AGENTS.md`:** буллеты external-review + designer-batch-checks соседствуют;
  хард-гейт хранилища, новые правила каталога, rewrite п. B про
  `syntaxcheck_file` — от апстрима. Файл 61 938 байт при потолке 62 000:
  форк-буллет external-review ужат, потолок НЕ поднимался. При следующем
  добавлении в AGENTS.md сначала освобождать бюджет.
- **`install.ps1`:** fork-список tools оставлен (с `koda`) + апстрим-константы
  `USE_EDT` / `SUPPORT_KEYS`; остальное слилось чисто (поддержка канала в
  init/update, новый validator-потолок). bash-канал (`scripts/install.sh`)
  апстримом не трогался; его список инструментов форк-шире (codex/kimi/koda).
  Новые команды/правила апстрима раскладываются generic-путями обоих каналов
  без правок инсталляторов — точечной проверки прогоном не было.
...
(в LocalProject3 `.dev.env` нет — модель не эмитится, дефолт клиента).

### 14. ✅ РЕШЕНО (2026-07-29): OpenSpec `schema: proposal` + pwsh для скилла

Две проблемы, вскрытые прогоном Codex в LocalProject1:
...
в 6 `.openspec.yaml` (LocalProject1: 3 активных + 2 архива; TestBaseExt: 1 архив).
...
`Documents/OrderServicesDemo.xml` прошёл; (2) в `SKILL.md` добавлена секция
