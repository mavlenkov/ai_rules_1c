...
- **Help-MCP-роутеры НЕ приняты.** Апстрим опустошил тела 18 правил
  (в т.ч. конфликтных `locks-and-transactions`, `platform-solutions`,
  `systematic-debugging`) и отправил читателя в `standards()` на
  `1C-docs-mcp`; корпус запинен ровно на наш merge-base `410951e`.
  Форк сохранил полные тексты (самодостаточность раскатки; Help MCP c этой
  коллекцией на Linux-машине не развёрнут). Три новых роутерных файла апстрима,
  приехавших авто-мержем (`help-corpus-retrieval.md` и указатели внутри новых
  правил) — приняты как документация; их триггер «Where this standard lives»
  в форк-копиях правил не встречается.
  ⚠ Корпус апстрима запинен на текст состояния `410951e` — то есть БЕЗ
  форк-фикса `НачалоТранзакции → НачатьТранзакцию` (`6f2c35d`). Форку чинить
  чужой корпус не нужно; при появлении ретрив-сценариев сверять retrieved
  текст с нашими файлами.
