...
- **Defaulted** (`INFOBASE_KIND`, `IB_USER`, `IB_PASSWORD`, `EXTENSION_NAME`, `EXPORT_PATH`, `LOG_PATH`, `NEW_OBJECTS_IN`, `IBCMD_CONFIG`, `SUBAGENT_MODEL_REASONING`, `SUBAGENT_MODEL_CODING`, `SUBAGENT_MODEL_LIGHT`) — empty resolves to a documented default; no question. In particular: empty `IB_USER` / `IB_PASSWORD` = no authentication (the `/N` / `/P` flags are simply omitted; an empty password is a valid configuration for dev / test infobases); empty `LOG_PATH` = `$env:TEMP\1cv8.log` (Windows) / `$TMPDIR/1cv8.log` (POSIX); empty `SUBAGENT_MODEL_REASONING` / `SUBAGENT_MODEL_CODING` / `SUBAGENT_MODEL_LIGHT` = subagents run on the AI client's default model (consumed by the installer only — see `content/rules/subagents.md → Model-tier routing`). Each tier may be overridden per AI client with the suffix `__<TOOL>` (`SUBAGENT_MODEL_CODING__OPENCODE`, `__CLAUDE_CODE`, `__CURSOR`, `__KILOCODE`; cascade `__<TOOL>` → bare tier → empty) — needed because Claude Code / Cursor take short aliases (`sonnet`) while OpenCode / Kilo Code need `provider/model` (`zai-coding-plan/glm-5.2`); for the latter a name without `/` is skipped with a WARNING. Re-ask `IB_USER` / `IB_PASSWORD` **only** if the command fails with an authentication error; re-ask `LOG_PATH` **only** if the resolved default path turns out to be non-writable.
