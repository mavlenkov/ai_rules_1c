...
**No `.dev.env` field is globally mandatory; guessing is prohibited.** Ask for `INFOBASE_PATH`, `PLATFORM_PATH`, or `INFOBASE_PUBLISH_URL` only when the current operation needs it. Never ask for advisory `PREFIX` / `COMPANY` / `DEVELOPER`; all other fields are Defaulted. Re-ask credentials only after authentication failure and `LOG_PATH` only when unwritable. Canonical classification and defaults — `content/rules/dev-standards-env.md §1`.

Fork routing: `SUBAGENT_MODEL_<TIER>__<TOOL>` overrides the bare tier for one client (`__OPENCODE`, `__CLAUDE_CODE`, etc.). Claude Code/Cursor accept aliases; OpenCode/Kilo require `provider/model` and skip invalid bare names with a warning. Linux deployment commands also read fork Section 5 (`CONVERTER_*`, `DB_SRV_*`, `REMOTE_*`).
...
References to `content/{rules,agents,skills,commands}/...` mean either the source path or the installed copy in the active tool's canonical project directory (`.cursor`, `.claude`, `.codex`, `.opencode`, `.kilo`, `.kimi-code`, `.qwen`, `.commandcode`, `.cline`, `.pi`, `.koda`, or `.ai-agent`). Match installed rules by file name because Cursor changes `.md` to `.mdc`. Source paths remain in rule text for portability; this convention applies globally.

### Infobase deployment (Linux + 1CFilesConverter — fork)

> Fork-specific (`mavlenkov/ai_rules_1c`). Commands `deploy-and-test`, `extensions`, `dataprocessors`, and `getconfigfiles` use `.dev.env` Section 5 and prefer 1CFilesConverter (`conf2ib.sh` / `ext2ib.sh`), with direct Designer as fallback. Their command files are the canonical executable procedures.

Linux connection strings are `/F<path>` and `/S<server:port\base>` with no space after the flag. Detect extensions only by `<ConfigurationExtensionPurpose>` in `Configuration.xml`; `<ConfigurationExtensionCompatibilityMode>` is not a marker. For MSSQL server infobases on Linux, the converter may proxy `ibcmd` through the configured `REMOTE_*` SSH host. Never reconstruct these flows from memory — follow the matching command and `.dev.env.example`.
...
Koda stores project settings under `.kodacli/`. Register servers with `koda mcp add <name> <url> --transport http --scope project`; after installing 1c-rules for Koda, use the generated commands in `.kodacli/mcp-setup.md`. For `1c-data-mcp`, set `INFOBASE_PUBLISH_URL` before generating or running the command.
...
Before writing or reviewing BSL or metadata, load `content/rules/coding-standards.md` — it is the single index of the detail files and the canonical place that lists them.
...
- **Koda CLI — no executable project subagent files.** Koda does not expose a project subagent-file format or an `InvokeAgent` tool. Installed `.koda/agents/*.md` files are reference prompts only; execute the selected role in the current agent unless a compatible extension supplies delegation.
- **Exploration — project `1c-explorer` only (hard gate).** When delegating read-only codebase / metadata exploration (locate code, map a subsystem, impact / callers, "where is X / how does Y work"), launch the project's **`1c-explorer`** subagent (`content/agents/explorer.md`, installed as the tool's custom agent — e.g. `.cursor/agents/explorer.md`). **Never** use the host tool's built-in Explore / `explore` / generic scout (Cursor Task `subagent_type: "explore"`, and equivalents). Built-ins cannot be overridden and do not follow the MCP-first chain or 1C report format — using them for project exploration is a defect. Narrow lookups the parent can answer with one direct MCP / read stay on the parent (no subagent). Canon — `content/rules/subagents.md → Host-tool built-in explorers`.
...
- **external-review** — automatic external critic (Codex) at the verification gate for non-trivial code: per-client detection, graceful fallback to `1c-code-reviewer`, rules context (`anti-patterns.md`, `coding-standards.md`, diff) attached; a second-opinion layer atop the mandatory hard validators, never a replacement (fork). Load at the verification gate.
