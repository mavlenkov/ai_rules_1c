...
- **Highly desirable** (`INFOBASE_PATH`, `PLATFORM_PATH` — IB-bound commands; `INFOBASE_PUBLISH_URL` — UI tests) — ask **only when the current task actually runs such an operation**; ask once and proceed. The fork's Linux + 1CFilesConverter deploy commands (`/deploy-and-test`, `/extensions`, `/dataprocessors`, `/getconfigfiles`) additionally read the `.dev.env` fork Section 5 (`CONVERTER_PATH`, `CONVERT_TOOL`, `DB_SRV_*`, `REMOTE_*`, …) — see `### Infobase deployment (Linux + 1CFilesConverter — fork)` below.
- **Defaulted** (everything else, incl. `IB_USER` / `IB_PASSWORD`, `UI_TESTING`, `SUBAGENT_MODEL_*`, `ORCHESTRATION`, `QUICKFIX_MAX_LINES`, `DEBUG_FAST_PATH`, `VERIFICATION_DEPTH`) — empty resolves to a documented default from `dev-standards-env.md §1`; no question. Re-ask credentials only after an authentication error; re-ask `LOG_PATH` only if the resolved path is non-writable. Each `SUBAGENT_MODEL_*` tier may be overridden per AI client with the suffix `__<TOOL>` (`SUBAGENT_MODEL_CODING__OPENCODE`, `__CLAUDE_CODE`, `__CURSOR`, `__KILOCODE`, `__KODA`; cascade `__<TOOL>` → bare tier → empty) — Claude Code / Cursor take short aliases (`sonnet`) while OpenCode / Kilo Code need `provider/model` (`zai-coding-plan/glm-5.2`); for the latter a name without `/` is skipped with a WARNING (fork).
...
Throughout this ruleset (this file, `content/rules/*.md`, `content/agents/*.md`, `content/skills/**/SKILL.md`, `content/commands/*.md`), any reference of the form `` `content/rules/<name>.md` ``, `` `content/agents/<name>.md` ``, `` `content/skills/<name>/SKILL.md` `` and the like means **either** the source-repo path (when the agent runs inside the `1c-rules` source repo) **or** the installed copy under the canonical rules directory of the active tool (`.cursor/rules/`, `.claude/rules-1c/`, `.codex/rules/`, `.opencode/`, `.kilo/rules-1c/`, `.kimi-code/rules-1c/`, `.qwen/rules-1c/`, `.commandcode/rules-1c/`, `.cline/rules-1c/`, `.pi/rules-1c/`, `.koda/rules/`, or `.ai-agent/rules/`). The file extension may change on install: Cursor stores rules as `.mdc` (`content/rules/<name>.md` → `.cursor/rules/<name>.mdc`) — when resolving a `<name>.md` reference in an installed project, match by file name, not by extension. The active tool reads the installed copy; rule files keep the source-repo path so they remain portable across tools.
...
Koda does **not** use a static MCP config file. Servers are added via `/mcp add-http` commands. After installing 1c-rules for Koda, check `.koda/mcp-setup.md` for ready-to-copy commands. For the `1c-data-mcp` server (URL derived from `INFOBASE_PUBLISH_URL`), run the command after setting the parameter. Koda names MCP tools as `<server>__<tool>` (e.g. `1c-code-metadata-mcp__metadatasearch`).
...
- **Koda CLI — subagents are JSON files.** When running under Koda, subagent definitions are in `.koda/agents/<name>.json` (not markdown). Each JSON has `name`, `system_prompt` (the markdown body from `content/agents/<name>.md`), `model` (resolved from `.dev.env`), `trust`, `allowed_tools`, `disallowed_tools`, `max_iterations`, and `skip_memory`. Use `/agent <name>` to switch to a subagent, or dispatch via `InvokeAgent` tool. Read-only agents (explorer, code-reviewer, arch-reviewer) have `trust: "plan"` and `skip_memory: true`.
...
- **external-review** — automatic external critic (Codex) at the verification gate for non-trivial code: trigger, per-client detection, graceful fallback to `1c-code-reviewer` on the current client's model, and the rules context (`anti-patterns.md` rubric + `coding-standards.md` index + diff) attached to Codex; a second-opinion layer on top of the mandatory hard validators, never a replacement (fork). Load at the verification gate of non-trivial code.
