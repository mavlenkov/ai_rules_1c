...
| Same for `IB_USER` / `IB_PASSWORD` / `PLATFORM_PATH` / `LOG_PATH` / `INFOBASE_PUBLISH_URL` when no IB-bound or UI-test step is **next** on the queue | Same rationale — premature. | Same: defer the dependent task block, proceed with everything else. |
