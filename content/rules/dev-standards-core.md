...
**Per-client resolution.** Model names are client-specific (Claude Code / Cursor: short aliases; OpenCode / Kilo Code: `provider/model`). The installer resolves each tier per target client by cascade `SUBAGENT_MODEL_<TIER>__<TOOL>` → `SUBAGENT_MODEL_<TIER>` → empty; for `provider/model` clients it skips a name without `/` and prints a WARNING. Tier semantics are billing-relative — per-token (Claude Code) = cost × capability; flat plan (OpenCode / Z.AI Coding Plan) = capability × latency (reasoning and coding may collapse to one model). See `content/rules/subagents.md → Model-tier routing` and `.dev.env.example §4`.
