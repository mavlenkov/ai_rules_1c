...
- **`INFOBASE_PATH` / `IB_USER` / `IB_PASSWORD` / `PLATFORM_PATH` / `LOG_PATH`** for the deploy / smoke-test block — do **not** ask pre-emptively when the IB-bound task block is not the **next** step. Mark the dependent block as `deferred-to-user` in `tasks.md`, finish every non-IB-bound task first, then state once in the closing summary that the deploy block is deferred until the value is provided.
