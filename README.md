...
|------------|------------|
| **metadatasearch** | Семантический/FTS поиск в метаданных XML. Фильтр по типу (`object_type`), компактный список (`names_only=true`) |
...
| Инструмент | Назначение |
|------------|------------|
| **search_forms** | Поиск по всем формам конфигурации (элементы, реквизиты, команды) |
| **inspect_form_layout** | Полное дерево элементов формы: иерархия, привязки данных, видимость, доступность, команды, обработчики событий |
...
- **forms_add.mdc** — создание и модификация форм через MCP-инструменты (`search_forms`, `inspect_form_layout`, `get_xsd_schema`, `verify_xml`) + компиляция через навык `1c-metadata-manage`
- **forms_events_add.mdc** — обработчики событий форм (globs: `**/Form.Module.bsl`)
