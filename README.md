...
- **Дополнительные параметры upstream** — `PLATFORM_ARGS` / `IBCMD_ARGS` передаются инструментам `1c-metadata-manage`, `SUPPORT_GUARD` управляет защитой объектов на поддержке, а `AGENT_MODEL` выбирает профиль головного агента. Пустой `VERIFICATION_DEPTH` теперь означает `standard`.
...
│   ├── commands/            # слэш-команды (включая rulesmodel, check-uuid, UI tooling; форк: extensions, dataprocessors)
