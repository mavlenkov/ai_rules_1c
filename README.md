...
Установка спроектирована как **протокол, который выполняет сам ИИ-агент**: вы даёте ему ссылку на репозиторий, агент клонирует его, читает [`AGENT-INSTALL.md`](AGENT-INSTALL.md) и `adapters/*.yaml`, после чего раскладывает файлы по проекту. Никакие внешние CLI и установщики при этом не обязательны.

Откройте проект в любимом ИИ-агенте (Cursor / Claude Code / Codex / OpenCode / Kilo Code) и отправьте примерно такое сообщение:

> Установи в этот проект правила `1c-rules` из `https://github.com/comol/ai_rules_1c`. Склонируй репозиторий во временную папку, прочитай `AGENT-INSTALL.md` и действуй по нему. Активные инструменты определи автоматически по содержимому проекта. Перед потенциально разрушительными действиями — спроси.

Если хотите явно перечислить инструменты, добавьте: «активные инструменты: cursor, claude-code» (любой набор из списка выше). Для обновления уже установленных правил формулировка та же — агент сам обновит существующую установку, опираясь на манифест `.ai-rules.json`.

### Fallback: PowerShell-установщик

Если агент по каким-то причинам не справляется (нет нужных инструментов, ограниченная среда, требуется детерминированный CI-friendly запуск) — реализация того же протокола поставляется как PowerShell-скрипт `install.ps1`. Можно попросить агента запустить его явно или сделать руками:
...
Команды установщика: `init` / `update` / `add <tool>` / `remove [<tool>]` / `doctor` / `eject`. Скрипт делает ровно то же, что и агент при ручной установке, но детерминированно и без обращения к LLM.

Подробности протокола, миграция существующих `AGENTS.md` / `CLAUDE.md` и поведение при конфликтах — в [`AGENT-INSTALL.md`](AGENT-INSTALL.md).
